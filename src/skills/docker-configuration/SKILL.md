...
description: Create and optimize Dockerfiles with multi-stage builds, alpine/distroless/scratch base images, layer caching optimization, build arguments, non-root user configuration, security hardening, image size reduction, BuildKit features. Write docker-compose.yml configurations for multi-container applications with service orchestration, networks, volumes, health checks, resource limits, depends_on, restart policies, environment variables, secrets management. Generate .dockerignore files to exclude build context. Configure development environments with hot reload, volume mounts, bind mounts. Set up production deployments with health checks, readiness probes, liveness probes, vulnerability scanning. Debug containers with docker exec, docker logs, docker inspect. Manage Docker networks, volumes, registries. Use when user mentions Docker, containers, Dockerfile, docker-compose, docker-compose.yml, docker-compose.yaml, docker-compose.override.yml, .dockerignore, containerization, containerize, dockerize, image optimization, multi-stage build, alpine, distroless, scratch image, base image, Docker security, docker build, docker run, docker exec, docker logs, docker ps, docker images, docker inspect, docker network, docker volume, docker-compose up, docker-compose down, docker-compose config, compose services, container orchestration, microservices deployment, CI/CD Docker, hadolint, trivy, docker scan, buildx, build cache, layer optimization, reduce image size, security scan, vulnerability scan, Docker Hub, private registry, ECR, GCR, push image, pull image, registry auth, 12-factor app, stateless containers, immutable infrastructure, bind mount, named volume, network isolation, bridge network, host network, troubleshoot Docker, debug containers, optimize build time, BuildKit cache, fix build errors, Dockerfile best practices.
