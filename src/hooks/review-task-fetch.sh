...
if [[ "${REVIEW_TASK_SELFTEST:-}" == 1 ]]; then
  ...
  two=$(extract_mr_urls "/review-task $one https://git.example.com/a/b/-/merge_requests/12")
  [[ "$two" == "$one"$'\n'"https://git.example.com/a/b/-/merge_requests/12" ]] || { echo "FAIL: multi mr url"; exit 1; }
